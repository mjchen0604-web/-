...
func (s *Service) registerModelsForAuth(a *coreauth.Auth) {
	...
	case "codex":
		...
		models = applyExcludedModels(models, excluded)
		models = applyCodexReasoningModelVariants(models, s.cfg)
	case "qwen":
...
func applyCodexReasoningModelVariants(models []*ModelInfo, cfg *config.Config) []*ModelInfo {
	if len(models) == 0 {
		return models
	}

	exposeVariants := cfg != nil && cfg.CodexSettings.ExposeReasoningModels
	...
	ensureBase := func(id string, sources ...string) {
		...
		for _, source := range sources {
			srcKey := strings.ToLower(strings.TrimSpace(source))
			if srcKey == "" {
				continue
			}
			src := modelIndex[srcKey]
			if src == nil {
				continue
			}
			clone := *src
			clone.ID = id
			if clone.DisplayName == "" || strings.EqualFold(clone.DisplayName, src.ID) {
				clone.DisplayName = id
			}
			if id == "codex-mini" {
				clone.DisplayName = "Codex Mini"
			}
			out = append(out, &clone)
			seen[key] = struct{}{}
			modelIndex[key] = &clone
			break
		}
	}

	// ChatMock keeps codex-mini as a base model; alias from the closest codex mini entry.
	ensureBase("codex-mini", "gpt-5-codex-mini", "gpt-5.1-codex-mini")
	...
	for _, group := range groups {
		baseKey := strings.ToLower(group.base)
		baseModel := modelIndex[baseKey]
		if baseModel == nil {
			continue
		}
		for _, effort := range group.efforts {
			id := group.base + "-" + effort
			...
			if baseModel.DisplayName != "" {
				clone.DisplayName = baseModel.DisplayName + " (" + effort + ")"
			} else {
				clone.DisplayName = id
			}
			out = append(out, &clone)
